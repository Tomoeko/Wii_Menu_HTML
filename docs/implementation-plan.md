...
| Channel management | Browser Channel Manager for creation, PNG/WAV upload, folder import, example install/repair, preview, Show/Hide and reviewed NAND updates with paired icon/banner previews; WAD/custom-package and NAND selection CLI; saved placement, fixed Disc, duplicate/invalid-data rejection and missing-layout/full-menu diagnostics. | Imported packages still need their own visual checks. Authored graphics are extensions, not native references. See [management](channel-management.md). |
...
| Keyboard | Original QWERTY/phone/symbol resources and cues; physical input; verified Settings profiles, candidate paging and long-text bounds; local first-party predictor using prepared word lists. | Native sequence comparisons, original Zi8 ranking and acceptance, learned state, manual arrow repeat, other WAD profiles and Mii remain open. See [keyboard notes](keyboard-source-notes.md). |
